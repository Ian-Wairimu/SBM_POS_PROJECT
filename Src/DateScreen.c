...
void dateScreenUseGoalResource(void)
{
  ...
  window = GL_GraphicLib_ReadResource(mGoal, RESOURCE_NAME);
  ...
  if (window == NULL)
  {
    ...
    GL_Dialog_Message(mGoal, "Error", message, GL_ICON_ERROR,
  ...
  {
    GL_Dialog_Message(mGoal, "Warning", "Canceled",
  ...
  else //OK button pressed
  {
    ...
    if (dateScreenSGetInputDate(window, &inputDate))
    {
      ...
      GL_Dialog_Message(mGoal, "Info", message,
